...
public class Novel_AcquaintanceModeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        inviting_friends.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(Novel_AcquaintanceModeActivity.this, android.R.style.Theme_Holo_Light_Dialog);
        ...
        start.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(name == null || theme == null || person_number == null || person_number.equals("")){
                    Toast.makeText(Novel_AcquaintanceModeActivity.this, "请输入名字，主题和人数", Toast.LENGTH_SHORT).show();
                }
                else{
                    Retrofit.Builder builder = new Retrofit.Builder()
                            .baseUrl("http://116.204.121.9:61583/")
                    ...
                    Call<create_return> call = novel_api.create(Login_LoginActivity.getToken(),message);
                                ...
                                if(create_return.getCode() == 200){
                                    ...
                                    for(int i = 0;i < target_friend.size();i++){
                                        ...
                                        inviting_reactionBody invitingBody
                                                = new inviting_reactionBody(newNovelId,"漂流小说",target_friend.get(i).getId(),User_Now.getUserNow().getUser().getId());
                                        Call<inviting_reactionReturn> inviting_return = novel_api.invite(Login_LoginActivity.getToken(),invitingBody);
                                        inviting_return.enqueue(new Callback<inviting_reactionReturn>() {
                                            ...
                                            public void onFailure(Call<inviting_reactionReturn> call, Throwable t) {
                                                Toast.makeText(Novel_AcquaintanceModeActivity.this, "邀请好友时发生错误，请检查网络", Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                    }
                                }
                            }else{
                                Toast.makeText(Novel_AcquaintanceModeActivity.this, "创建失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                        @Override
